...
- **JSON shapes:** `main/boost_json.c/.h` is the single owner of every JSON object the device serves (`/state`, `/config`, `/themes`, `/sensors/calibration`, `/network`, TPMS config). Both transports call it: `boost_web.c` (HTTP/WebSocket) and the BLE Control route in `boost_app_ble.c`. HTTP/BLE parity is therefore structural, not a byte-for-byte copy to keep in sync.
- **Timezone table:** `web/tz.json` is the canonical POSIX zone list. The web dropdown fetches it; both companion apps generate their curated presets from the entries carrying IANA `ids`. POSIX offsets are west-positive, so an east-of-Greenwich zone carries a minus (Sydney is `AEST-10AEDT,...`) — matches the firmware `tz_from_offset()` fallback. Regenerate embedded assets after editing `web/`.
