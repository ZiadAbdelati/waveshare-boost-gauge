...
{
    char json[768];
    const int n = boost_json_calibration(json, sizeof(json));
    return n > 0 && n < (int)sizeof(json) ? send_json(req, json) : ESP_FAIL;
}
...
static void state_ws_push(void *arg)
{
    ...
    const int n = boost_json_state(current, sizeof(current));
...
{
    char json[512];
    boost_json_config(json, sizeof(json));
    return send_json(req, json);
}
...
static esp_err_t themes_get(httpd_req_t *req)
{
     ...
     * and the whole dashboard failed to parse it. boost_json_themes() reports
     * truncation instead, and 414 is the same guard now the builder owns the
     * comma placement too. */
    const size_t cap = 1024 + boost_theme_count() * BOOST_JSON_THEME_MAX;
    char *json = calloc(1, cap);
    if (json == NULL) {
        return send_err(req, HTTPD_500, "themes_alloc");
    }
    const int n = boost_json_themes(json, cap);
    if (n < 0) {
        free(json);
...
{
    char json[128];
    boost_json_tpms_config(json, sizeof(json));
    return send_json(req, json);
}
...
{
    char json[512];
    boost_json_network_status(json, sizeof(json));
    return send_json(req, json);
}
