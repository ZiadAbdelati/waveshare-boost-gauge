...
//===========================================================================
//
// PROJECT-OWNED COPY of LVGL's bundled AnimatedGIF decoder.
//
// Upstream: managed_components/lvgl__lvgl/src/libs/gif/AnimatedGIF/src/gif.c
// See boost_gif_dec.h for why this lives here. Divergences are marked "BOOST:".
//
// Summary of divergences:
//   1. Framebuffer layout. Upstream allocates TWO planes:
//        [0 .. w*h)            8-bit palette-index plane
//        [w*h .. w*h*(1+bpp))  fully "cooked" pixels
//      In GIF_DRAW_COOKED mode the index plane is written for every pixel and
//      never read back: the only reader is DrawNewPixels(), which runs
//      exclusively in GIF_DRAW_RAW mode, and transparency/disposal in cooked
//      mode work purely by *not* writing the cooked pixel. The widget hardcodes
//      GIF_DRAW_COOKED, so the index-plane stores are pure waste (434 KB of
//      PSRAM write traffic per 466x466 frame) and the plane itself wastes
//      217 KB of PSRAM. The index-plane stores are removed and the cooked plane
//      now starts at offset 0.
//      The framebuffer therefore holds exactly ONE plane whose meaning depends
//      on ucDrawType: the index plane in GIF_DRAW_RAW, the cooked plane in
//      GIF_DRAW_COOKED. Callers must size it accordingly.
//   2. Turbo decoder compiled out (GIF_SUPPORT_TURBO == 0).
...
#endif
        {
            rc = DecodeLZW(pGIF, 0); // BOOST: always the standard decoder, see boost_gif_dec.h
        }
        if (rc != 0) // problem
            return 0;
    }
...
//
// BOOST: upstream also maintained an 8-bit palette-index copy of the canvas here
// (the `d8` pointer). In GIF_DRAW_COOKED mode nothing ever reads it back - the
// cooked plane *is* the persistent canvas, transparency is "leave the old cooked
// pixel alone" and disposal-to-background is "write the background colour" - so
// every one of those stores has been removed. See the banner at the top.
    ...
    if (pPage->ucPaletteType == GIF_PALETTE_1BPP || pPage->ucPaletteType == GIF_PALETTE_1BPP_OLED) { // 1-bit mono
        ...
        int iPitch = 0;
         if (pPage->ucPaletteType == GIF_PALETTE_1BPP) { // horizontal pixels
             d = pPage->pFrameBuffer; // BOOST: cooked plane now starts at offset 0
...
// BOOST: everything between here and the end of DecodeLZWTurbo() is the Turbo
// LZW path. It is unreachable in this project (nothing assigns pTurboBuffer)
// and it still assumes the upstream two-plane framebuffer layout, so it is
// compiled out rather than left as a landmine. See boost_gif_dec.h.
#if GIF_SUPPORT_TURBO
//
// LZWCopyBytes
//
...
// BOOST: the overrun is bounded by GIF_LINEBUF_SLACK - see boost_gif_dec.h.
// BOOST: may_alias types, see boost_gif_dec.h.
                BIGUINT tmp = *(const gif_u32_alias_t *) s;
                s += sizeof(BIGUINT);
                *(gif_u32_alias_t *)buf = tmp;
                buf += sizeof(BIGUINT);
#else
                *buf++ = *s++;
#endif
        ...
        else  /* Pixels cross into next line */
        {
            ...
            {
                // BOOST: the cooked plane starts at offset 0 now that the dead
                // 8-bit index plane is gone (upstream started it at w*h).
                int iPitch = 0, iBpp = 1, iOffset = 0;
