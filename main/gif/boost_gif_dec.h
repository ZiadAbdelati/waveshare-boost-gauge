...
/*
 * PROJECT-OWNED COPY of LVGL's bundled AnimatedGIF decoder header.
 *
 * Upstream: managed_components/lvgl__lvgl/src/libs/gif/AnimatedGIF/src/AnimatedGIF.h
 *
 * `managed_components/` is git-ignored and is rewritten by any dependency
 * refresh, so edits made there are silently lost (this has already happened
 * once to this project - see README "Intentional local LVGL changes"). The
 * whole GIF layer therefore lives here instead. The upstream copy is excluded
 * from the build in the top-level CMakeLists.txt.
 *
 * Divergences from upstream are all marked "BOOST:".
 */

#ifndef BOOST_GIF_DEC_H
#define BOOST_GIF_DEC_H
...
// to 3 bytes. For ucLineBuf the worst case start is index (iWidth - 1), so the
// last byte written is index (iWidth + 2), and iWidth <= iCanvasWidth <=
// MAX_WIDTH (enforced by GIFInit()). This slack makes that overrun land inside
// the array for *any* accepted canvas width, independently of the separate
// BOOST_MEDIA_STORE_MAX_DIMENSION upload cap (which boost_gif.c also asserts
// against MAX_WIDTH).
#define GIF_LINEBUF_SLACK 8
...
// GIFMakePels() copies one byte at a time. The ESP32-S3 (Xtensa LX7) sets
// XCHAL_UNALIGNED_LOAD_HW = XCHAL_UNALIGNED_STORE_HW = 1 and
// XCHAL_UNALIGNED_{LOAD,STORE}_EXCEPTION = 0 (verified in
// components/xtensa/esp32s3/include/xtensa/config/core-isa.h), i.e. unaligned
// 16/32-bit access is handled in hardware for both DRAM and cached PSRAM.
// x86/x86-64 hosts (sim builds) are likewise unaligned-safe.
#if !defined(ALLOWS_UNALIGNED) && (defined(__XTENSA__) || defined(__i386__) || defined(__x86_64__) || defined(_M_IX86) || defined(_M_X64))
    #define ALLOWS_UNALIGNED
#endif

// BOOST: the wide accesses below all read/write `unsigned char` buffers through
// a wider lvalue, which is a strict-aliasing violation and ESP-IDF compiles at
// -O2 with -fstrict-aliasing. may_alias makes it well defined instead of
// relying on the optimiser not noticing.
#if defined(__GNUC__) || defined(__clang__)
    #define GIF_MAY_ALIAS __attribute__((may_alias))
#else
    #define GIF_MAY_ALIAS
#endif
