...
| Theme system (web half landed; firmware pending) | A "theme" was only a palette swap over one arc face, and the dashboard chrome re-skinned with it. Wanted four visually distinct gauge designs, selectable and persistent. | Rendering was hard-coded to a single arc layout; the theme carried colors only. | Promoted the theme to a `style` discriminator (`arc`/`vault`/`hud`/`bigdigit`). `web/app.js` dispatches `drawGauge()` on `activeThemeStyle()` to per-style renderers; `setTheme()` now drives only the gauge palette so the console chrome stays fixed. `tools/mock_server.py` serves four themes (`dyno-cell`, `vault-tec`, `night-city`, `big-digit`), each with a `style`. Alvida Fatface OTF added under `main/fonts/` and inlined as base64 `@font-face` in `web/styles.css`. Shared rules: filled arcs reference `zeroAngle`; `big-digit` uses tabular fixed-slot digits with the decimal pinned to face center. Verified host-only in-browser via mock_server; embedded assets regenerated. | **Firmware NOT ported yet.** Next task: add `style` + the four themes to `boost_theme.c/.h`; emit `style` in `boost_web.c`; dispatch per-style build/update/teardown in `boost_gauge.c` without regressing the arc DMA/partial-refresh path; generate + embed the Alvida LVGL font via `lv_font_conv`. Needs ESP-IDF 5.5.1 (not installed on the current dev PC — only `esptool` is present). Do not claim hardware behavior until built, flashed, and cadence-guarded. |